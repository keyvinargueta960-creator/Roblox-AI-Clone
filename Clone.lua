...
--------------------------------------------------------------------------------
-- 2. THE DIRECT GOOGLE API CONNECTION ENGINE (PORTED FROM GITHUB SOURCE)
--------------------------------------------------------------------------------
local function askAI(msg)
    -- Looks for Delta/Wave hidden executor background request engines
    local executorRequest = request or (http and http.request) or (syn and syn.request)
    if not executorRequest then 
        return "Your mobile executor is missing custom network privileges!" 
    end
    
    -- Direct endpoint connection used in the working script
    local directGoogleUrl = "https://googleapis.com" .. ACTIVE_KEY
        ...
        contents = {
            {
                role = "user",
                parts = {
                    { text = "System Prompt: You are a friendly player clone. Keep answers under 2 short sentences. Talk like a real human player chatting in Roblox. User Message: " .. msg }
                }
            }
        }
    }
    ...
    return "Direct connection failed. The AI engine is experiencing high traffic!"
end

-- Anti-Kick Loop to avoid getting disconnected while chatting
